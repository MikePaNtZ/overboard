...
|---|---|---|
| **Q1** | Is the 4K-page, no-`NO_HZ_FULL` packaged RT kernel good enough for a 2 ms loop on Pi 5? **Unknowable without hardware.** | Proceed with the packaged kernel; the parent design §4's ladder is the response if AC-5 fails |
...
| **Q12** ✅ **CLOSED 2026-08-05** | How does Pi 5 firmware select the v8-rt kernel, given it defaults to `kernel_2712.img`? **Operational, not architectural** — but it is the difference between a card that boots and one that does not. | **Answered by observation, not by the restore test.** `kernel=kernel8.img` in `config.txt` overrides firmware auto-selection on a Pi 5. Measured on a Pi 5 Rev 1.1, stock Raspberry Pi OS: `uname -r` moved from `6.18.34+rpt-rpi-2712` to `6.18.34+rpt-rpi-v8` across one reboot, with `auto_initramfs=1` resolving the matching initramfs unassisted. This is the directive `device/overboard-pi5/config.txt` already ships under `[pi5]`, so the image's existing assumption is confirmed rather than changed. See [`runbook-pi-first-boot.md`](runbook-pi-first-boot.md) §5. **Residual, tracked separately:** this proves the *selection mechanism* on the 6.18 non-RT kernel. It does not prove the pinned 6.12.75 v8-**rt** package installs itself as `kernel8.img` — the filename `kernel=` depends on. `verify_kernel.sh` asserts modules and the DTB but not the boot-image filename; adding `kernel8.img` to `KERNEL_REQUIRED_FILES` would close that in CI with no hardware. Q11 (Pi-5-specific RT regression) is untouched and remains open. |
